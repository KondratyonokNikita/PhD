...
\begin{proof}
    ...
    Если $R$ -- факториальное кольцо и $\Lambda_R \in [0, 1)$.
    Рассмотрим произвольный элемент $\frac{a}{b} \in \zeroless{F_1}$, где $a, b \in \zeroless{R}$, $(a, b) = 1$.
    Тогда имеем $\left|\frac{a}{b}\right| = \frac{\elementnorm{a}}{\elementnorm{b}} < 1$.
...
%    TODO
    \begin{center}
        \begin{large}
            Тут кажется используем мультипликативностть нормы, хотя она не обязана быть.
            А именно, если $(a, b) \neq 1$, то рассуждения выше нельзя проводить, так как $\left|\frac{a}{b}\right| \neq \frac{\elementnorm{a}}{\elementnorm{b}}$.
        \end{large}
    \end{center}
\end{proof}
...
\begin{proof}
    ...
    Следоваельно, выполнено $E(d) \subseteq V(\lambda, 1)$.
    ...
    Предположим, что для любого $r \in (0, 1)$ это не выполняется и $E(d) \setminus V(\lambda, r) = X_r \neq \emptyset$.
    Рассмотрим произвольную последовательность $r_n$, для которой $\lim_{n \to \infty} r_n = 1-0$.
    Для каждого элемента этой последовательности можно выбрать точку $q_{r_n} \in X_{r_n}$.

    Так как последовательность точек $q_{r_n}$ ограничена, то существует сходящаяся подпоследовательность $q_{r_{n_m}}$.
    Пусть $\lim_{m \to \infty} q_{r_{n_m}} = q$.
    Заметим, что $V(\lambda, 1) = \cup_{m \in \mathbb{N}} V(\lambda, r_{n_m})$.
    Следовательно, $E(d) \setminus V(\lambda, 1) = \cap_{m \in \mathbb{N}} X_{r_{n_m}}$.

    Из того, что $q_{r_{n_m}} \in X_{r_{n_m}}$ и множество $E(d) \setminus V(\lambda, 1)$ замкнуто в $\mathbb{R} \times \mathbb{R}$ следует, что $q \in E(d) \setminus V(\lambda, 1)$.
    Это противоречит тому, что $E(d) \setminus V(\lambda, 1) = \emptyset$.

%    TODO
    \begin{center}
        \begin{large}
            Тут совсем непонятно почему $q \in E(d) \setminus V(\lambda, 1)$.
            Имеем, что $X_{r_1} \subset X_{r_2}$, если $r_2 < r_1$.
            Вроде выглядит как правда, до как формально доказать непонятно.
            Да и вообще в предложении выше сказано, что это критерий и там указана четко граница $1$ и непонятно почему ее вообще можно было бы уменьшить.
        \end{large}
    \end{center}
