...
\begin{algorithm}\label{algorithm:R_in_T}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item Если такие элементы не нашлись и $l+1 \ge D_R$, то вернуть ''выберите большие $D_R$ и $M$'' и завершить алгоритм
            
            \item Если такие элементы не нашлись и $l+1 < D_R$, то добавить в множество $\mathbb{L}$ элемент $(x_0,\ldots,x_l,x_{l+1})$
...
\end{lemma}
\begin{proof}
    \textbf{Correctness of Algorithm 2.}
    Suppose that the answer of Algorithm 2 is "Yes".
    Take arbitrary $x_0\in\mathbb{K}$.
    If $x_0\in\mathbb{J}$, then the triple $(x_0,\alpha,D_\mathbb{K})$ is regular for any $\alpha\in\mathbb{F}^*$ and $D_\mathbb{K}\in\mathbb{N}$.

    Then we want to show that the triple $(x_0,\alpha,D_\mathbb{K})$ is regular for any $x_0\in\mathbb{K}^*\setminus\mathbb{J}$ and $\alpha\in\mathbb{F}^*$.
    Let us consider the set $\mathbb{L}_1$ of all elements that were put in $\mathbb{L}$.
    If $(x_0,\ldots,x_k)\in\mathbb{L}_1$, $(y_0,\ldots,y_l)\in\mathbb{L}_1$ and $x_i=y_i$ for any $i=\overline{1,k}$, $k\le l$, then we will delete $(x_0,\ldots,x_k)$ from $\mathbb{L}_1$. Let us fix $x_0$ and show that $(x_0,\alpha,D_\mathbb{K})$ is regular. It is easy to see that if $(x_0,\alpha,k)$ is regular, then $(x_0,\alpha,k+i)$ is regular for any $i\in\mathbb{N}$.

    Let us consider an element $(x_0,\ldots,x_l)\in\mathbb{L}_1$.
    This element generates the set of $\alpha$ such that 
    $$x_l=\left(\left(\ldots\left(\left(\alpha-x_0\right)^{-1} -x_1\right)^{-1}-\ldots\right)^{-1}-x_{l-1}\right)^{-1}.$$
    It is obvious that all such elements of $\mathbb{L}_1$ with fixed $x_0$ generates $\mathbb{F}^*$.
    Since the answer of Algorithm 2 is "Yes" we can find sequences from the Algorithm for each element of $\mathbb{L}_1$ with fixed $x_0$.
    So the triple $(x_0,\alpha,D_\mathbb{K})$ is regular for any $x_0, \alpha$.
    And the first part of the definition of the set $\mathcal{T}$ holds.

    Suppose that $D_\mathbb{K}\ge3$. Proof of correctness of Steps 10-13 is completely analogous to the proof of correctness of Steps 1-9.
\end{proof}
