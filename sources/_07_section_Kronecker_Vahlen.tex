...
\begin{center}
    ...
    \section*{ГЛАВА \arabic{section}.\\ ТЕОРЕМА КРОНЕКЕРА-ВАЛЕНА В ФАКТОРИАЛЬНЫХ КОЛЬЦАХ}\label{section:Kronecker-Vahlen theorem}
...
\subsubsection{Упрощенный метод проверки $R \in \mathcal{T}$}
...
\subsubsection{Примеры проверки $R \in \mathcal{T}$}
...
\begin{example}\label{example:thm1:P[t]}
    Пусть $R = \mathbb{P}[t]$, где $\mathbb{P}$ -- поле.
...
\begin{example}\label{example:thm1:coordinate_ring_of_circle}
	Пусть $R$ это координатное кольцо $\mathbb{Q}[i][x, y]/(x^2 + y^2 + 1)$.
...
\begin{example}\label{example:Z[sqrt{-11}]}\cite{source:Rolletschek_1990}
...
\subsubsection{Общий метод проверки $R \in \mathcal{T}$}
...
\begin{proof}
    ...
    Используя код, приведенный в приложении А получаем контрпримеры для всех $\mathbb{Q}(\sqrt{d})$, которые приведены в приложении Б и опубликованы в работе~[\ref{source:JSC_2021}].
\end{proof}
...
В работе~\cite{source:Selfridge} доказано следующее полезное утверждение.
...
\begin{proposition}\label{proposition:fundamental_in_circle}
    ...
    Для $r > 0$ и $\lambda = \lambda_1 + \lambda_2 \sqrt{d} \in \mathbb{Z}[\sqrt{d}]$
    \begin{equation*}
        U(\lambda, r) = \left\{
