...
\begin{example}
    ...
    \begin{itemize}
        ...
        Кольцо $\mathbb{Z}_K$, образованное целыми алгебраическими элементами этого поля, является дедекиндовым с конечным полем остатков.
...
\begin{statement}[\cite{source:Petukhova} (Обобщенная теорема Эйлера)]\label{statement:euler_function}
...
Пусть $R = \mathbb{Z}_K$~-- кольцо целых алгебраических элементов поля $K$.
...
\begin{statement}[\cite{source:Lezowski}]\label{statement:orbit}
    ...
    \end{equation*}
    Тогда для любых $\varepsilon \in \invertible{\mathbb{Z}_K}$, $Z \in \Phi(\mathbb{Z}_K)$ выполнено
    \begin{equation*}
...
\end{proposition}
\begin{proof}
    В работе Поста~\cite{source:Post}, доказано, что $\Nm{\ideal{a}} = |\mathrm{det}(A)|$, где $A$ матрица соответствующая $\mathbb{Z}$-представлению $\ideal{a}$.
...
\end{proposition}
\begin{proof}
    Сравнение $a \equiv b \pmod{\ideal{a}}$ выполняется тогда и только тогда, когда $a - b \in \ideal{a}$.
    Эта проверка сводится к проверке разрешимости систем линейных уравнений размера $n \times n$ с коэффициентами размера $O(L)$, что может быть выполнено за $\tilde{O}(\log L)$ бинарных операций.
\end{proof}
