...
%     В алгоритме~\ref{algorithm:miller_rabin} выбирается произвольный идеал $\ideal{a} \subseteq R$ и вычисляется $\ideal{r}_0 = \ideal{a}^u \pmod{\ideal{n}}$.
%     Это требует $O(\log u)$ арифметических операций.
%     Далее не более $t$ раз для $\ideal{r}_i$ вычисляется $\ideal{r}_i^2 \pmod{\ideal{n}}$.
